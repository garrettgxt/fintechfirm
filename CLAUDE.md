...
- This is the site's PRIMARY funding entry point ("Add funds" in the
...
- IMPORTANT OPEN ISSUE (unchanged by the NOWPayments removal below): this
  is a deliberate custodial funding path. A user pays crypto they already
  own to Coinstate Capital's own address, and gets an internal USD
  balance credited in return — Coinstate Capital actually receives and
  holds this money. This directly contradicts the site's own footer/
  Auth.jsx copy ("Coinstate Capital does not hold customer funds, crypto,
  or securities" / "never has access to it"). The user was told this
  explicitly and chose to ship it anyway with the copy unchanged — a
  known, deliberately-deferred inconsistency, not an oversight. Revisit
  before real users/real money: either update that copy or drop this
  feature. Also worth real legal advice before going live — holding and
  converting customer crypto into internal credit is a different, more
  regulated business than a referral-only on-ramp.
- NOWPAYMENTS REMOVED ENTIRELY (2026-09-06): came up because a real $2
  test payment didn't land where the user expected. Root cause turned
  out to be a real misunderstanding of the old flow — NOWPayments
  generated a unique `pay_address` per invoice, and where funds went
  *after* NOWPayments received them (forwarded to a payout wallet, held
  as balance, whatever) was entirely a NOWPayments-dashboard setting, not
  anything in this codebase. The user first asked to hardcode a specific
  ETH address (0xfBe9Dc46f9985B1dA483D3e4FA7F65F5fa82946F, the same one
  already in src/walletAddresses.js from the old Banxa setup) as the
  NOWPayments payout wallet — the safe, no-code-change option, explained
  and agreed to. Then the user changed direction and asked to drop
  NOWPayments from the flow completely and use that fixed address
  directly. Before implementing, the user was walked through and
  explicitly accepted the real tradeoff of removing NOWPayments: no
  automated payment confirmation is possible for a fixed, shared address
  (none of BTC/ETH/LTC/SOL have a memo/payment-ID field), so nothing can
  auto-credit anymore. The user chose the recommended replacement:
  customer submits what they paid (+ optional tx hash) after sending, an
  admin verifies it on a block explorer and approves it from /admin.
- Current flow: src/components/CreditInvoiceModal.jsx (amount + currency
  picker — preset chips at $20/$50/$100/$250/$500/$1000 plus a free-text
  custom amount field, and an optional `initialCurrency` prop so a
  chart's "Buy" button can preselect a coin) → shows the FIXED address +
  QR for that currency from src/walletAddresses.js's `WALLET_ADDRESSES`
  (same address for every customer paying in that currency — this is the
  live Site Credit deposit flow now, not just Banxa dev/test data
  anymore) → customer submits amount + optional tx hash via
  functions/submit-deposit-request.js (inserts a `pending` row in the new
  `credit_deposit_requests` table, credits nothing yet) → admin reviews
  pending requests in /admin (functions/admin-list-deposit-requests.js
  lists them, with a block-explorer link built from the tx hash) and
  clicks Approve/Reject → functions/admin-review-deposit.js is the ONLY
  place a real balance is ever incremented now (via the same
  `increment_credit_balance` Postgres function NOWPayments' webhook used
  to call) → functions/get-credit-balance.js stays as the read-only
  balance endpoint, unchanged.
- Deleted: functions/create-payment.js, functions/nowpayments-webhook.js,
  functions/payment-status.js (no remaining callers). The OLD
  `credit_payments` Supabase table (NOWPayments' invoice audit trail) was
  left in place as historical data, not dropped — `user_credits` (the
  actual balance table) and `increment_credit_balance` are unchanged and
  still actively used. New table: `credit_deposit_requests` (wallet_
  address, amount_usd, currency, tx_hash, status ['pending'|'approved'|
  'rejected'], created_at, reviewed_at) — supabase/migrations/
  20260906070000_add_credit_deposit_requests.sql.
  NOWPAYMENTS_API_KEY / NOWPAYMENTS_IPN_SECRET Cloudflare env vars are now
  unused (harmless to leave set, or remove them from the dashboard).
...
- Actively used, not optional — table wallet_overrides (wallet_address,
  ...
  demo_positions back Demo Mode above; user_credits and the newer
  credit_deposit_requests back Site Credit above (credit_payments is old
  NOWPayments audit-trail data, left in place, no longer written to).
- Accessed server-side only via SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY
  ...
  admin-reset-demo.js, admin-list-deposit-requests.js,
  admin-review-deposit.js, get-override.js, register-wallet.js,
  submit-deposit-request.js, get-credit-balance.js, demo-trade.js,
  get-demo-portfolio.js.
