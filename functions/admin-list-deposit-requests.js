// Cloudflare Pages Function: lists Site Credit deposit requests for the
...
export async function onRequest(context) {
  ...
  const res = await fetch(
    `${supabaseUrl}/rest/v1/credit_deposit_requests?select=*${filter}&order=created_at.desc`,
    { headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` } }
  );

  if (!res.ok) {
    return new Response(JSON.stringify({ error: "Failed to fetch deposit requests" }), { status: 500 });
  }
