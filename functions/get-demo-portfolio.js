...
// positions. The frontend computes current market value itself from
// quotes it already has (live crypto prices / polled stock-forex quotes)
// rather than this function fetching prices too — avoids duplicate calls.
...
export async function onRequest(context) {
  ...
  try {
    const [overrideRes, positionsRes] = await Promise.all([
      fetch(
    ...
    if (!overrideRes.ok || !positionsRes.ok) {
